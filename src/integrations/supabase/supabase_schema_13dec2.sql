...
CREATE TABLE public.fssai_expiry_notifications (
  ...
  seller_id uuid NOT NULL,
  notification_type character varying NOT NULL CHECK (notification_type::text = ANY (ARRAY['seller_kyc_fssai'::character varying, 'product_fssai'::character varying]::text[])),
  kyc_fssai_number character varying,
  kyc_fssai_expiry_date date,
  listing_id uuid,
  variant_id uuid,
  product_name character varying,
  variant_name character varying,
  variant_fssai_number character varying,
  variant_fssai_expiry_date date,
  days_until_expiry integer NOT NULL,
  notification_status character varying NOT NULL DEFAULT 'pending'::character varying CHECK (notification_status::text = ANY (ARRAY['pending'::character varying, 'acknowledged'::character varying, 'renewed'::character varying, 'dismissed'::character varying]::text[])),
  notification_message text NOT NULL,
  priority character varying NOT NULL DEFAULT 'medium'::character varying CHECK (priority::text = ANY (ARRAY['low'::character varying, 'medium'::character varying, 'high'::character varying, 'critical'::character varying]::text[])),
  created_at timestamp with time zone DEFAULT now(),
  acknowledged_at timestamp with time zone,
  renewed_at timestamp with time zone,
  CONSTRAINT fssai_expiry_notifications_pkey PRIMARY KEY (id),
  CONSTRAINT fssai_expiry_notifications_seller_id_fkey FOREIGN KEY (seller_id) REFERENCES public.sellers(id),
  CONSTRAINT fssai_expiry_notifications_listing_id_fkey FOREIGN KEY (listing_id) REFERENCES public.seller_product_listings(listing_id),
  CONSTRAINT fssai_expiry_notifications_variant_id_fkey FOREIGN KEY (variant_id) REFERENCES public.listing_variants(variant_id)
);
...
CREATE TABLE public.listing_variants (
  ...
  nutrient_table_image_url text,
  fssai_label_image_url text,
  fssai_number character varying CHECK (fssai_number IS NULL OR fssai_number::text ~ '^[0-9]{14}$'::text),
  fssai_expiry_date date,
  nutrient_breakdown jsonb,
  accuracy_attested boolean DEFAULT false,
  attested_by uuid,
  attested_at timestamp with time zone,
  ingredient_list jsonb CHECK (ingredient_list IS NULL OR jsonb_typeof(ingredient_list) = 'object'::text AND ingredient_list ? 'ingredients'::text),
  allergen_info jsonb CHECK (allergen_info IS NULL OR jsonb_typeof(allergen_info) = 'object'::text AND (allergen_info ? 'allergen_ids'::text OR allergen_info ? 'custom_allergens'::text OR allergen_info ? 'contains_allergens'::text)),
  ingredient_image_uploaded_at timestamp with time zone,
  nutrient_table_image_uploaded_at timestamp with time zone,
  fssai_label_image_uploaded_at timestamp with time zone,
  is_p0_compliant boolean DEFAULT ((ingredient_list IS NOT NULL) AND (allergen_info IS NOT NULL) AND (ingredient_image_url IS NOT NULL) AND (nutrient_table_image_url IS NOT NULL) AND (fssai_label_image_url IS NOT NULL) AND (fssai_number IS NOT NULL) AND (nutrient_breakdown IS NOT NULL) AND (nutrient_breakdown ? 'energyKcal'::text)),
  ...
  CONSTRAINT listing_variants_attested_by_fkey FOREIGN KEY (attested_by) REFERENCES public.sellers(id)
);
...
CREATE TABLE public.seller_documents (
  id uuid NOT NULL DEFAULT gen_random_uuid(),
  seller_id uuid,
  doc_type text CHECK (doc_type = ANY (ARRAY['selfie'::text, 'aadhaar'::text, 'pan'::text, 'gstin'::text, 'fssai_certificate'::text])),
  storage_path text,
  file_name text,
  file_size bigint,
  mime_type text,
...
CREATE TABLE public.seller_product_listings (
  ...
  status character varying DEFAULT 'draft'::character varying CHECK (status::text = ANY (ARRAY['draft'::character varying::text, 'active'::character varying::text, 'inactive'::character varying::text, 'suspended'::character varying::text, 'pending_approval'::character varying::text, 'failed_approval'::character varying::text, 'expired'::character varying::text])),
  is_verified boolean DEFAULT false,
...
CREATE TABLE public.seller_variant_edit_logs (
  ...
  ip_address inet,
  user_agent text,
  CONSTRAINT seller_variant_edit_logs_pkey PRIMARY KEY (log_id),
  CONSTRAINT seller_variant_edit_logs_variant_id_fkey FOREIGN KEY (variant_id) REFERENCES public.listing_variants(variant_id),
  CONSTRAINT seller_variant_edit_logs_changed_by_fkey FOREIGN KEY (changed_by) REFERENCES public.sellers(id)
);
...
CREATE TABLE public.sellers (
  ...
  created_at timestamp with time zone DEFAULT now(),
  updated_at timestamp with time zone DEFAULT now(),
  fssai_license_number text NOT NULL DEFAULT 'NA'::text,
  fssai_license_expiry_date date NOT NULL DEFAULT CURRENT_DATE,
...
CREATE TABLE public.variant_change_log (
  log_id uuid NOT NULL DEFAULT gen_random_uuid(),
  variant_id uuid NOT NULL,
  listing_id uuid NOT NULL,
  ...
  change_type text,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT variant_change_log_pkey PRIMARY KEY (log_id)
);
