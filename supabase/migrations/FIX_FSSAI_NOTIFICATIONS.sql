-- =====================================================
-- FIX: FSSAI Notifications System
-- =====================================================
-- Run this if notifications aren't showing up
-- =====================================================

-- Step 1: Drop and recreate table to fix foreign key constraint
DROP TABLE IF EXISTS public.fssai_expiry_notifications CASCADE;

-- Creating fssai_expiry_notifications table
CREATE TABLE public.fssai_expiry_notifications (
    ...
    listing_id UUID REFERENCES public.seller_product_listings(listing_id) ON DELETE CASCADE,
...
-- Add indexes
CREATE INDEX idx_fssai_notifications_seller ON public.fssai_expiry_notifications(seller_id);
CREATE INDEX idx_fssai_notifications_status ON public.fssai_expiry_notifications(notification_status);
CREATE INDEX idx_fssai_notifications_priority ON public.fssai_expiry_notifications(priority);
CREATE INDEX idx_fssai_notifications_created ON public.fssai_expiry_notifications(created_at);
CREATE INDEX idx_fssai_notifications_variant ON public.fssai_expiry_notifications(variant_id);
...
-- Step 2: Drop and recreate functions to ensure they're up to date
DROP FUNCTION IF EXISTS public.check_seller_kyc_fssai_expiry() CASCADE;
DROP FUNCTION IF EXISTS public.check_product_fssai_expiry() CASCADE;
DROP FUNCTION IF EXISTS public.check_all_fssai_expiry() CASCADE;
DROP FUNCTION IF EXISTS public.get_seller_fssai_notifications(UUID, VARCHAR) CASCADE;
DROP FUNCTION IF EXISTS public.update_fssai_notification_status(UUID, VARCHAR) CASCADE;
...
BEGIN
    ...
    SELECT 
        ...
        n.days_until_expiry,
        n.created_at,
        n.product_name::VARCHAR,
        n.variant_name::VARCHAR,
        n.kyc_fssai_number::VARCHAR,
        n.variant_fssai_number::VARCHAR
...
-- =====================================================
-- Step 3: Run initial check to generate notifications
-- =====================================================
...
BEGIN
    SELECT COUNT(*) INTO notification_count FROM public.fssai_expiry_notifications;
    RAISE NOTICE '✅ Setup complete. Total notifications: %', notification_count;
END $$;
